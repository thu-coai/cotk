...
def get_accuracy(loss_arr, label_key='label', prediction_key='prediction'):
	if label_key in loss_arr[0] and prediction_key in loss_arr[0]:
		batch_result = list(map(lambda x: \
						(x[label_key] == x[prediction_key]).float().detach().cpu().numpy().tolist(), \
					loss_arr))
		return np.mean(list(chain(*batch_result)))
	else:
		return 0
